...
var PlayerTable = /** @class */ (function () {
    ...
    PlayerTable.prototype.setupActiveLordsPanel = function (lords) {
        var _this = this;
        // Create the active lords panel HTML
        var activeLordsContainer = document.getElementById("player-panel-".concat(this.playerId, "-active-lords"));
        if (!activeLordsContainer) {
            // If the container doesn't exist, create it dynamically
            var playerPanel = document.getElementById("player-panel-".concat(this.playerId));
            var sentinelsSection = playerPanel.querySelector('.sentinels');
            // Create the active lords section
            var activeLordsSection_1 = dojo.create("div");
            var title = _("Active Powers");
            activeLordsSection_1.innerHTML = "<h4>" + title + "</h4>" +
                "<div id=\"player-panel-" + this.playerId + "-active-lords\" class=\"active-lords\"></div>";
            // Insert after the sentinels section
            sentinelsSection.parentNode.insertBefore(activeLordsSection_1, sentinelsSection.nextSibling);
            activeLordsContainer = document.getElementById("player-panel-".concat(this.playerId, "-active-lords"));
        }
        // Get active lords (lords that are free, have special effects, and are not disabled)
        var activeLords = lords.filter(function (lord) {
            return lord.location == null &&
                _this.isSpecialLord(lord.lord_id) &&
                !_this.freeLords.getCardElement(lord).classList.contains('disabled');
        });
        // Hide the entire section if there are no active lords
        var activeLordsSection = activeLordsContainer.parentNode;
        ...
        activeLords.forEach(function (lord) {
            var lordElement = dojo.create("div");
            lordElement.className = "active-lord-item";
            var lordIcon = dojo.create("i");
            lordIcon.className = "icon icon-lord";
            lordIcon.style.color = _this.getLordFactionColor(lord.lord_id);
            lordElement.appendChild(lordIcon);
            var lordText = dojo.create("span");
    ...
    PlayerTable.prototype.getLordEffectText = function (lordId) {
